...
\begin{exercise}
	...
	\begin{enumerate}[label=(\alph*)]
		...
		\item Show that this game satisfies part~\ref{item:spm_game:compact} of the definition of a game of strategic complements if we additionally assume that for each fixed $a_{-i} \in \R_+$, demand is more elastic the higher $i$'s own price, and eventually elastic: formally, $a_i \mapsto \abs*{ \eps_i(a_i,a_{-i}) }$ is increasing and $\lim_{a_i \uparrow +\infty} \abs*{ \eps_i(a_i,a_{-i}) } > 1$.
...
\begin{proposition}
	...
	%
	Let $(I,(A_i,u_i)_{i \in I})$ be a finite game of strategic complements. Let $\underline{a} \in A$ and $\widebar{a} \in A$ be (respectively) its least and its greatest pure-strategy Nash equilibria, and let $X^\infty \subseteq A$ be the set of rationalisable action profiles. Then $\underline{a}_i = \min X^\infty_i$ and $\widebar{a}_i = \max X^\infty_i$ for each player $i \in I$.
	%
\end{proposition}
...
\begin{proof}
	...
	Since $\underline{a}$ and $\widebar{a}$ are pure-strategy Nash equilibria, they belong to $X^\infty$, so
	%
	\begin{equation}
		\underline{a} \geq \min X^\infty
		\quad \text{and} \quad
		\widebar{a} \leq \max X^\infty  .
		\label{eq:spm_rbty3}
	\end{equation}
	%
	On the other hand, by definition of the sets $\left( X^{t-1} \right)_{t \in \N}$ and of the functions $\underline{\text{br}}$ and $\widebar{\text{br}}$, and since these functions are increasing, we have
	%
	\begin{equation*}
		...
		= \max X^t
		\quad \text{for each $t \in \N$.}
	\end{equation*}
	%
	Since $A$ is finite, there is a $T \in \N$ such that $X^{T-1}=X^T=X^\infty$. Hence $\min X^\infty$ is a fixed point of $\underline{\text{br}}$ and $\max X^\infty$ is a fixed point of $\widebar{\text{br}}$. It follows that $\min X^\infty$ and $\max X^\infty$ are pure-strategy Nash equilibria, which by definition of $\underline{a}$ and $\widebar{a}$ implies that
	%
	\begin{equation}
		\underline{a} \leq \min X^\infty
		\quad \text{and} \quad
		\widebar{a} \geq \max X^\infty .
		\label{eq:spm_rbty1}
	\end{equation}
	%
	Combining \eqref{eq:spm_rbty3} and \eqref{eq:spm_rbty1} yields $\underline{a} = \min X^\infty$ and $\widebar{a} = \max X^\infty$.
	%
\end{proof}
